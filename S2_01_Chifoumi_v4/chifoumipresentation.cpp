...
void ChifoumiPresentation::FinDePartie(char p_gagnant)
{
    unsigned int scoreGagnant = _leModele->getScoreObjectif();
    if (p_gagnant=='J')
    {
        _laVue->afficherMsBFinDePartie("Vous",scoreGagnant);
        _laVue->desactiverBoutons();
    }

    if (p_gagnant=='M')
    {
        _laVue->afficherMsBFinDePartie("Machine",scoreGagnant);
        _laVue->desactiverBoutons();
    }
...
void ChifoumiPresentation::jouerTour()
{
    ...
    _laVue->majInterfaceGlobale(_leModele->getEtat(),
                                ...
                                _leModele->getScoreMachine());
    _laVue->changerCouleurJoueur('B');
    FinDePartie(_leModele->determinerObjectifAtteint());
}
