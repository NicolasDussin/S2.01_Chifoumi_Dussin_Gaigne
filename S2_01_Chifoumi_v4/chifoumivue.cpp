...
void ChifoumiVue::afficherMsBAPropos()
{
    ...
    MsBAPropos->setText("S2.01 Chifoumi V4 par DUSSIN Nicolas et GAIGNE Lilian");
    MsBAPropos->setStandardButtons(QMessageBox::Ok);
    MsBAPropos->exec();
}

void ChifoumiVue::afficherMsBFinDePartie(QString p_nomGagnant, unsigned int p_scoreGagnant)
{
    QMessageBox *MsBFinDePartie=new QMessageBox;
    MsBFinDePartie->setWindowTitle("Fin de partie");
    MsBFinDePartie->setIcon(QMessageBox::Information);
    MsBFinDePartie->setText("Bravo " + p_nomGagnant + " ! Vous gagnez avec " + QString::number(p_scoreGagnant) + " points.");
